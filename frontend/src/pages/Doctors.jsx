...
function Doctors() {
 ...
 const {doctors} = useContext(AppContext)
  const  applyFilter  = ()=>{
    if(speciality){
      const filterdoctors = doctors.filter(doc => doc.speciality === speciality)
  ...
  return (
    <div>
      ...
      <div className='flex flex-col sm:flex-row items-start gap-5 mt-5'>
        <button className={`py-1 px-3 border rounded text-sm transition-all sm:hidden ${showFilter  ? 'bg-primary text-white' : ''} `} onClick={()=>{
            ...
            filterDoc.map((item, index) => (
              ...
              >
                ...
                <div className="p-4">
                  ...
                  {
                item.available ?<>  <p className="w-2 h-2 bg-green-500 rounded-full"></p>
                <p>Available</p></> : <>
