...
function Myappointments() {
  ...
  const getUserAppointments = async () => {
    ...
    try {
      const { data } = await axios.get("https://doctora-appointments-api.vercel.app/api/user/appointments", { headers: { token } });
  ...
  const cancelAppointment = async(appointmentId)=>{
    ...
    try{
        const  {data} = await  axios.post("https://doctora-appointments-api.vercel.app/api/user/remove-appointment" ,{appointmentId}, {headers: {token}})
        ...
        {appointments && appointments.map((item, index) => (
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
            ...
            <div className='flex flex-col gap-2 justify-end'>
              ...
              {item.cancelled && <button className='sm:min-w-48 border py-2 border-red-500 rounded text-red-500'>Appointment cancelled</button> }  
            </div>
          </div>
        ))}
      </div>
